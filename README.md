...
- **Local dev stuck redirecting to production after Google:** Supabase only redirects to URLs listed under **Redirect URLs**. The app sends `redirectTo = <origin>/auth/callback?next=/staff` where `<origin>` is your browser tab unless you set **`NEXT_PUBLIC_STAFF_OAUTH_REDIRECT_ORIGIN`** (e.g. `http://localhost:3000`) in `.env.local` to match exactly what you whitelisted. On `/staff/login` (dev), the gray box shows the exact callback URL to add in Supabase.
